...
module tt_um_array_mult_vga (
 ...
 bin2bcd_8bit bin2bcd_8bit_inst(.X(P),.Hundred(P_hundreds),.Tens(P_tens),.Ones(P_ones));
  ...
  wire logo_pixels = ((hpos >= CARD_X0 ) && (hpos < CARD_X0 + 512)) &&( (vpos >= MULT_START + 8*(bit_size+bit_gap) + bit_size) && (vpos < 128 + MULT_START + 8*(bit_size+bit_gap) + bit_size));
  wire [9:0] hpos_adj = hpos - CARD_X0;
  wire [9:0] vpos_adj = vpos -  (MULT_START + 8*(bit_size+bit_gap) + bit_size);
  
  mastodon_rom_compress   mastodon_rom_inst (.x(hpos_adj[8:0]), .y(vpos_adj[6:0]) ,.pixel(pixel_color)); 
